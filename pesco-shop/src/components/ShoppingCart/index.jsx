...
export default function ShoppingCart({
  ...
  useEffect(() => {
    if (data?.data?.price && onPriceCalculated) {
      const total = quantity * data.data.price;
      onPriceCalculated(productId, total);
    }
  }, [data, quantity]);
  ...
  if (isError) {
    return (
      <div className="h-screen flex flex-col justify-center items-center">
        ...
        <h1 className="font-bold animate-pulse text-center">
          an error has been accured{error.message}
        </h1>
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4 border p-4 rounded-xl shadow-md bg-white hover:shadow-lg transition">
        ...
        <div className="flex-1 w-full text-right">
          ...
          <p className="text-green-600 font-semibold mt-2">
            مجموع:{(quantity * data?.data?.price).toFixed(2)}$
          </p>
          <button
            className="mt-4 text-sm text-white bg-red-500 hover:bg-red-600 px-4 py-1 rounded-md transition cursor-pointer"
            onClick={() => removeProductCompletely(productId)}
          >
            حذف محصول
          </button>
        </div>
